...
This fork of Cranelift allows using `cranelift-jit` as well as various related
parts of Cranelift in a `no_std` environment. 

It is quite hacky and not well made, however it (should) hopefully be usable.

This also includes a modified `regalloc`, which is the only out-of-repo dependency that
does not support `no_std` on it's own.

To use `cranelift-jit`, you need to provide a `MemoryManager`. See `jit/src/lib.rs`.

Thank you to the Cranelift team for allowing this to be possible with a lot of work
already done (many crates were already `no_std`) and for Cranelift in general.

**The original Cranelift README is below.**
